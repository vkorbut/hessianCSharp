...
namespace hessiancsharp.io.hessian2
{
	...
	public class CSerializerFactory
	{
        ...
        public static void addCustom(Type type, AbstractSerializer cSerializer, AbstractDeserializer cDeserializer)
		{
			m_htSerializerMap[type] = cSerializer;
			m_htDeserializerMap[type] = cDeserializer;
            m_htTypeMap[type.FullName] =  cDeserializer;
		}

        public static AbstractSerializer getCustomSer(Type type)
        {
            return (AbstractSerializer) m_htSerializerMap[type];
        }
        public static AbstractDeserializer getCustomDeser(Type type)
        {
            return (AbstractDeserializer) m_htDeserializerMap[type];
        }
	    ...
	    public static CSerializerFactory newInstance()
	    {
            if (instance == null) instance = new CSerializerFactory();
	        return instance;
	    }

        public static void setInstance(CSerializerFactory i)
        {
            instance = i;
        }

	    private static CSerializerFactory instance;
	    private AbstractDeserializer _arrayListDeserializer;
	    ...
	    public AbstractDeserializer GetObjectDeserializer(string type, Type cl)
	    {
            ...
            if (cl == null || cl.Equals(reader.GetOwnType()) || reader.GetOwnType().IsSubclassOf(cl) || reader.ReadResolve || reader.GetOwnType().IsSubclassOf(typeof(HessianHandle)))
            {
                return reader;
            }
            ...
            {
                log.fine("hessian: expected deserializer '" + cl.Name + "' at '" + type + "' (" + reader.Type.Name + ")");
            }*/

            return GetDeserializer(cl);
	    }

	    public AbstractDeserializer GetListDeserializer(string type, Type cl)
	    {
            AbstractDeserializer reader = GetListDeserializer(type);

            if (cl == null || cl.Equals(reader.GetOwnType()) || reader.GetOwnType().IsSubclassOf(cl))
            {
                return reader;
            }
            ...
            /*if (log.isLoggable(Level.FINE))
            {
                log.fine("hessian: expected '" + cl.Name + "' at '" + type + "' (" + reader.Type.Name + ")");
            }*/
	    ...
	    private AbstractDeserializer GetListDeserializer(string type)
	    {
            AbstractDeserializer deserializer = GetDeserializer(type);
	    ...
	    public virtual AbstractSerializer GetObjectSerializer(Type cl)
	    {
            AbstractSerializer serializer = GetSerializer(cl);
